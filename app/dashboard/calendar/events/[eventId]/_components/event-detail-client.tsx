...
export function EventDetailClient({ event: initialEvent, eventId, currentUserId }: EventDetailClientProps) {
  ...
  // Set breadcrumb label on mount
  useEffect(() => {
    setCustomLabel(eventId, event.title)
    return () => {
      setCustomLabel(eventId, null)
    }
  }, [eventId, event.title, setCustomLabel])
